...
# Material B is full 3D: all 6 components are active — no special handling needed.
# Components: [ε₁₁, ε₂₂, ε₃₃, ε₁₂, ε₂₃, ε₁₃] and [σ₁₁, σ₂₂, σ₃₃, σ₁₂, σ₂₃, σ₁₃]
...
plt.title('Material B — Training and Test Loss vs Epochs')
plt.legend()
plt.tight_layout()
plt.savefig('outputs/Material_B_loss.png', dpi=150)
print('Saved: outputs/Material_B_loss.png')
...
for tag, sample_idx in [('median', median_idx), ('worst', worst_idx)]:
    ...
    plt.suptitle(
        f'Material B — Ground-truth vs Predicted Stress '
        f'({tag}-error test sample, idx={sample_idx}, RMSE={rmse_total:.3g} Pa)',
        fontsize=11
    )
    plt.tight_layout()
    fname = f'outputs/Material_B_stress_{tag}.png'
...
plt.ylabel('Count')
plt.title(f'Material B — Distribution of Test RMSE across {NTEST} test samples')
plt.legend()
plt.tight_layout()
plt.savefig('outputs/Material_B_error_dist.png', dpi=150)
plt.close()
print('Saved: outputs/Material_B_error_dist.png')
